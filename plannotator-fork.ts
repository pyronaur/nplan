...
/* nplan does not carry forward upstream assistant-message parsing for
 * checklist completion and last-message annotation.
...
export default function plannotator(pi: ExtensionAPI): void {
	...
	function updateStatus(ctx: ExtensionContext): void {
		/* nplan does not keep upstream status/checklist rendering.
	...
	function updateWidget(ctx: ExtensionContext): void {
		/* nplan does not keep upstream widget/checklist rendering.
		if (phase === "executing" && checklistItems.length > 0) {
	...
	*/
	...
	/* nplan does not carry forward upstream review/annotation/archive commands.
	...
	*/
	...
	pi.registerTool({
		...
		async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
			...
			// nplan does not carry forward upstream checklist parsing.
...
Execute each step in order. After completing a step, include [DONE:n] in your response where n is the step number.`,
	...
	/* nplan does not carry forward upstream turn-end checklist tracking or
	...
	*/
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		/* nplan does not carry forward upstream execution-state rebuild from
		 * checklist parsing and [DONE:n] scanning.
		if (phase === "executing") {
			const fullPath = resolvePlanPath(ctx.cwd);
